...
if strmatch(gal, 'REG10') eq 1 then mwlines = [mwlines, 1327., 1329., 1525., 1648., 1903., 1922]
...
chisreg = where(mask eq 1  and s.wave gt 1240. and s.wave lt 2000 and finite(s.flux) eq 1 and s.err ne 0, comp = badc)
; and  $ ;,comp = badc); and $ ;These are galaxy specific masks
   ;(s.wave lt 1440 or s.wave gt 1455) , comp = badc)
s.err[badc] = 0

plot, s.wave, s.flux, yr = [0,2], xr = [1200, 2000]
djs_oplot, s.wave[chisreg], s.flux[chisreg], color = 'green'

vdisp = cspeed/2000.*2.355
...
dfpsplot,  '/Users/johnchisholm/Documents/M101/'+gal+'/he2.ps', /color, /create
!p.font = 0
!p.multi = [0, 1, 2]
angstrom = cgsymbol('angstrom')
...
printf, lun, '# # The fit was only made from 1240-2200, while masking out between 1327., 1329., 1525., 1648., 1903., 1922A. Be skeptical of the fit outside of these wavelengths.'
