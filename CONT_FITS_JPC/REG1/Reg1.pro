...
if strmatch(gal, 'REG1') eq 1 then mwlines = [mwlines, 1065., 1067., 1069.,  1080.,1645.,1389., 1385., 1381., 1383., 1481., 1490.]
...
chisreg = where(mask eq 1  and s.wave gt 1300. and s.wave lt 2050 and finite(s.flux) eq 1 and s.err ne 0, comp = badc)
; and  $ ;,comp = badc); and $ ;These are galaxy specific masks
   ;(s.wave lt 1440 or s.wave gt 1455) , comp = badc)
s.err[badc] = 0

plot, s.wave, s.flux, yr = [0,2], xr = [1300, 2050]
...
print, stddev(ebv), stddev(met), stddev(age)/1e6
...
!p.multi = [0, 1, 2]
plot, s.wave, s.flux, xr = [1475, 1575], yr = [0, 2], thick = 2
djs_oplot, s.wave[chisreg], s.flux[chisreg], color = 'green'
...
plot, s.wave, s.flux, xr = [1230, 1255], yr = [0, 2]
djs_oplot, s.wave, cfit004, color = 'red'
djs_oplot, s.wave, cfit001, color = 'green'
djs_oplot, s.wave, cfit008, color = 'orange'
djs_oplot, s.wave, cfit_super, color = 'blue', thick = 2
...
dfpsplot, '/Users/johnchisholm/Documents/M101/'+gal+'/'+gal+'C4-bestfit-vel.ps', /color, /create
!p.font = 0
!p.multi = [0, 1, 2]
...
dfpsplot, '/Users/johnchisholm/megasaura/'+gal+'/BPASS/N5-best-fit.ps', /color, /create
!p.font = 0
!p.multi = [0, 1, 2]
angstrom = cgsymbol('angstrom')
plot, flux.wave[where(flux.flux gt 0 and flux.flux lt 5)], smooth(flux.flux[where(flux.flux gt 0 and flux.flux lt 5)], 2), yr = [0, 2], xr = [1230, 1280], psym = 10, /xs,$
...
plot, flux.wave[where(flux.flux gt 0 and flux.flux lt 5)], smooth(flux.flux[where(flux.flux gt 0 and flux.flux lt 5)], 2)/smooth(cfit[where(flux.flux gt 0 and flux.flux lt 5)],2), yr = [0, 2], xr = [1230, 1280], psym = 10, /xs,$
  xtit = 'Rest Wavelength ('+angstrom+')', ytit = 'Normalized Flux', charsize = 1.5, charthick = 5, thick = 10
djs_oplot, [1200, 10000], [1, 1], thick = 7
dfpsclose


dfpsplot, '/Users/johnchisholm/Documents/M101/'+gal+'/'+gal+'S4-bestfit.ps', /color, /create
!p.font = 0
!p.multi = [0, 1, 2]
angstrom = cgsymbol('angstrom')
...
dfpsplot, '/Users/johnchisholm/Documents/M101/'+gal+'/'+gal+'photo.ps', /color, /create
!p.font = 0
!p.multi = [0, 1, 2]
angstrom = cgsymbol('angstrom')
...
dfpsplot, '/Users/johnchisholm/Documents/M101/'+gal+'/'+gal+'full.ps', /color, /create
!p.font = 0
!p.multi = [0, 1, 2]
angstrom = cgsymbol('angstrom')
...
dfpsplot, '/Users/johnchisholm/Documents/M101/'+gal+'/'+gal+'rix.ps', /color, /create
!p.font = 0
!p.multi = [0, 1, 2]
angstrom = cgsymbol('angstrom')
...
printf, lun, '# # The fit was only made from 1240-2200, while masking out between 1065., 1067., 1069.,  1080.,1645.,1389., 1385., 1381., 1383., 1481., 1490. A. Be skeptical of the fit outside of these wavelengths.'
for i =0, n_elements(flux.wave)-1 do printf, lun, flux.wave[i], flux.flux[i], flux.err[i], fit_super[i]
free_lun, lun

openw, lun,    '/Users/johnchisholm/Documents/M101/'+gal+'/'+gal+'.dat''-BPASS-fit.txt', /get_lun
printf, lun, '# # BPASS Continuum fits (v2.1.1) to '+gal+' with a nebular continuum model  and log(U) = -2.5.'
printf, lun, '# # Created on (yyyy-mm-dd)' + date
printf, lun, '# # The fit was created using the parameters in the '+gal+'continuum-parameters.fits file'
printf, lun, '# # The first column is the wavelength, the second and third columns are the observed flux and error (in f_lam) normalized by the median flux between 1267-1276, the last column is the linear-combination BPASS fit to the flux.'
printf, lun, '# # The velocity dispersion (sigma) used to create this template is ' + strtrim(string(vdisp), 2) + ' km/s'
printf, lun, '# # The stellar continuum is offset by '+ strtrim(string(newvoff.voff), 2) + ' km/s'
printf, lun, '# # The fit was only made from 1240-2200, while masking out between 1284, 1321, 1328, 1341, 1353, 1440, 1443, 1503, 1595, 1693, 1847, 1987, 1990, 1993, 1996, 2103, 2273 A. Be skeptical of the fit outside of these wavelengths.'
for i =0, n_elements(flux.wave)-1 do printf, lun, flux.wave[i], flux.flux[i], flux.err[i], fit_super_b[i]
free_lun, lun
...
output_b = {ebv: b_super_mods.ebv, ebv_err: b_super_mods.ebv_err, weighted_z: exz_b, $
            weighted_age: exage_b/10.^6,  light_frac: b_super_mods.light_Frac/total(b_super_mods.light_frac), $
            light_frac_err: b_super_mods.light_frac_err/total(b_super_mods.light_frac), model_age: bp_mod.age, $
            model_z: bp_mod.z}  
